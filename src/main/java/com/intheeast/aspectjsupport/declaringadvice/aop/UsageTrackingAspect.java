...
public class UsageTrackingAspect {
    ...
    @DeclareParents(value = "com.intheeast.aspectjsupport.declaringadvice.service.*+", defaultImpl = DefaultUsageTracked.class)
    public static UsageTracked mixin;

	// (..): 메서드의 파라미터 개수와 타입에 상관없이 일치합니다. 즉, 파라미터가 0개일 수도 있고, 여러 개일 수도 있습니다.
    @Before("execution(* com.intheeast.aspectjsupport.declaringadvice.service.*.*(..)) && this(usageTracked)")
